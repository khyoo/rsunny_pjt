...
public class ForsaleRestController {
	...
	public Map<String, List<PositionDTO>> getSalePositions(SaleSearchDTO form, Model model) throws Exception {
		...
		if (!StringUtils.isEmpty(form.getSaleDetailType()))
			form.setSaleDetailTypeList(Arrays.asList(form.getSaleDetailType().split(",")));
		if (!StringUtils.isEmpty(form.getTradeType()))
			form.setTradeTypeList(Arrays.asList(form.getTradeType().split(",")));
	...
	public SaleDTO saleView(@PathVariable String rid, SaleDTO form, Model model) throws Exception {
		...
		if (data != null) {
			...
			String linkUrl = String.format("%s/02la01?t=%s&rid=%s&w=%s", SessionUtil.getDomainUrl(), data.getSaleType(),
					data.getRid(), URLEncoder.encode(data.getAddr1(), "UTF-8"));
	...
	public ResultDTO save(@PathVariable String pagetype, @PathVariable String rid, SaleDTO form, Model model)
			throws Exception {
	...
	public ResultDTO delete(@PathVariable String pagetype, @PathVariable String rid, SaleDTO form, Model model)
			throws Exception {
	...
	public UserDTO agentInfo(UserDTO form, Model model) throws Exception {
		...
		if (data == null)
			data = UserDTO.of(form.getUserid());
	...
	public List<SaleDTO> getRealTradingsListFromPnu(SaleDTO form, Model model) throws Exception {
		...
		if (form.getSaleDetailType().equals("a1") && form.getTradeType().equals("A")) {
			rtlist = service.getRealTradingsAptListFromPnu(form);
		} else if (form.getSaleDetailType().equals("a1") && !form.getTradeType().equals("A")) {
			rtlist = service.getRealTradingsApt2ListFromPnu(form);
		} else if (form.getSaleDetailType().equals("a2") && form.getTradeType().equals("A")) {
			rtlist = service.getRealTradingsOfficetelListFromPnu(form);
		} else if (form.getSaleDetailType().equals("a2") && !form.getTradeType().equals("A")) {
			rtlist = service.getRealTradingsOfficetel2ListFromPnu(form);
		} else if (form.getSaleDetailType().equals("a3")) {
			rtlist = service.getRealTradingsPresaleListFromPnu(form);
		} else if (form.getSaleDetailType().equals("h1") && form.getTradeType().equals("A")) {
